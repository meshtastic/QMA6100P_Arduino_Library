 ...
 * - NEWDATA_X: 1 if X-channel data has been updated since last reading, 0
 * otherwise.
 */
typedef struct {
  uint8_t newdata_x : 1;
 ...
 * - NEWDATA_Y: 1 if Y-channel data has been updated since last reading, 0
 * otherwise.
 */
typedef struct {
  uint8_t newdata_y : 1;
 ...
 * - NEWDATA_Z: 1 if Z-channel data has been updated since last reading, 0
 * otherwise.
 */
typedef struct {
  uint8_t newdata_z : 1;
 ...
 * - ANY_MOT_SIGN: 1 if the sign of any_motion triggering signal is negative, 0
 * if positive.
 * - ANY_MOT_FIRST_Z: 1 if any_motion interrupt is triggered by the Z axis, 0
 * otherwise.
 * - ANY_MOT_FIRST_Y: 1 if any_motion interrupt is triggered by the Y axis, 0
 * otherwise.
 * - ANY_MOT_FIRST_X: 1 if any_motion interrupt is triggered by the X axis, 0
 * otherwise.
 */
typedef struct {
  uint8_t no_mot : 1;
 ...
 * - STEP_INT: 1 if step valid interrupt is active, 0 if step quit interrupt is
 * inactive.
...
/**
 * @brief Union to access the full scale range and filter settings or the entire
 * byte.
 */
typedef union {
  uint8_t all;
 ...
 * @details
 * - MODE_BIT: 1 to set the device into active mode, 0 to set it into standby
 * mode.
...
/**
 * @brief Union to access the interrupt mapping settings for INT1 pin or the
 * entire byte.
 */
typedef union {
  uint8_t all;
...
/**
 * @brief Union to access the interrupt mapping settings for INT2 pin or the
 * entire byte.
 */
typedef union {
  uint8_t all;
...
#define SFE_QMA6100P_SR                                                        \
  0x36 // writing 0xB6, soft reset all of the registers.
...
0b00 BYPASS
RAISE_XYZ_SW<2:0>
0x3E[2:0]: User can select the acceleration data of which axis to be stored in
the FIFO. This configuration can be done by setting FIFO_CH, where ‘111b’ for
x-, y-, and z-axis, ‘001b’ for x-axis only, ‘010b’ for y-axis only, ‘100b’ for
z-axis only
*/
typedef struct {
  uint8_t fifo_mode : 2;
...
/*
FIFO_DATA<7:0>: FIFO read out data. User can read out FIFO data through this
register. Data format depends on the setting of FIFO_CH (0x3e<2:0>). When the
FIFO data is the LSB part of acceleration data, and if FIFO is empty, then
FIFO_DATA<0> is 0. Otherwise if FIFO is not empty and the data is effective,
