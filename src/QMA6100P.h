/**
 * @brief The following class implements the methods to set, get, and read from
 * the Triple Axis Accelerometer - QMA6100P.
 */
...
public:
   ...
   */
  bool readRegisterRegion(uint8_t registerAddress, uint8_t *sensorData,
                          int len);
   ...
   *
   * @param userAccel Pointer to the structure to store the converted
   * accelerometer data.
   * @param rawAccelData Pointer to the structure holding the raw accelerometer
   * data.
   ...
   *
   * @param rawAccelData Pointer to the structure to store the raw accelerometer
   * data.
   * @return bool True if the operation is successful, false otherwise.
   */
...
protected:
  ...
  TwoWire *_i2cPort;
  /**
   * @brief keeps track of I2C address. set in .begin()
   */
